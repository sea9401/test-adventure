...
자동 배포 중에는 앱을 직접 노출하지 않고 정적 `503` 업데이트 안내를 표시한다. 새 앱의
DB health와 빌드 SHA 검증이 모두 통과한 경우에만 안내를 해제한다. 배포가 실패하면 이전
빌드를 복원하며, 복원한 앱의 health가 돌아오지 않으면 안내 화면을 계속 유지한다.
