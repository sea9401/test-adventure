...
- [x] 협동 보스 비용·공개 범위와 7차·수행 규칙을 요구하는 실패 테스트를 추가한다.
- [x] `npm test -- src/app/manual/current-content.test.tsx`로 문구 누락 실패를 확인한다.
- [x] `coop.tsx`와 `jobs.tsx`를 실제 상수와 UI 동작에 맞춰 수정한다.
...
- [x] 6분야·7단계·20,000점·대표 3종·랭킹을 요구하는 실패 테스트를 추가한다.
- [x] 집중 테스트의 예상 실패를 확인한다.
- [x] 세 안내서에 기능의 위치, 진행 방식, 보상 원칙과 공개 범위를 추가한다.
- [x] 집중 테스트를 다시 실행해 통과를 확인한다.
...
- [x] 원정 일괄 진행, 게시판 Lv.20, 거래 제한, 장비 등록 상태, 스킬 상세와 틱 표시를 요구하는 실패 테스트를 추가한다.
- [x] 집중 테스트의 예상 실패를 확인한다.
- [x] 관련 안내서 본문과 섹션 요약을 최소 범위로 갱신한다.
- [x] 집중 테스트를 다시 실행해 통과를 확인한다.
...
- [x] 기능·편의·수정 사항과 숙소 제외 범위를 패치 노트로 작성한다.
- [x] 안내서 테스트 전체와 `sections.test.ts`를 실행한다.
- [x] TypeScript, 수정 파일 ESLint, 프로덕션 빌드와 `git diff --check`를 실행한다.
- [x] 변경 목록을 재검토하고 숙소 노출 문구가 없는지 확인한 뒤 커밋한다.
