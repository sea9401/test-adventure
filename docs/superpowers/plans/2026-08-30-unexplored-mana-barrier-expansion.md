# Unexplored Mana Barrier Expansion Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 결계 수호체·룬 집행자·봉인 감시체를 실제 미개척지 사냥에 균등 출현시키고 기존 전투 능력·집중 효과·보상·이미지를 연결한다.

**Architecture:** 기존 풀별 `releaseStage`와 몬스터 ID 기반 런타임 구조를 재사용한다. 마력 방벽체를 `expanded`로 전환하면 조우·서버 사냥 준비·집중 효과·풀 공통 보상은 기존 공통 경로를 따르며, 런타임 이미지 맵에 새 몬스터 두 종만 추가한다.

**Tech Stack:** TypeScript, Vitest, Next.js 정적 이미지 자산, Sharp 기반 프로젝트 이미지 최적화/검사 스크립트

## Global Constraints

- 철갑 군단과 마력 방벽체만 `expanded`; 나머지 10개 풀은 `launch` 상태를 유지한다.
- 마력 방벽체 세 몬스터는 각각 1/3 확률로 출현한다.
- 기존 시뮬레이션 수치, 능력, 자동 전투 규칙을 변경하지 않는다.
- 세 몬스터는 같은 `v2_unexplored_mana_barrier_material`과 드롭 확률을 사용한다.
- 신규 이미지는 512×512 WebP, 실제 RGBA 알파 배경, 상아색 석재·금색 테두리·청색 마력의 판타지 골렘 스타일이다.
- 배포, 병합, 푸시, PR 생성은 하지 않는다.

---

### Task 1: 마력 방벽체 활성화와 런타임 배선
...
- Produces: 마력 방벽체의 활성 ID `barrier_guardian | rune_executor | seal_watcher`
- Produces: `SPECIAL_IMAGE_PATHS.rune_executor`, `SPECIAL_IMAGE_PATHS.seal_watcher`

- [x] **Step 1: 풀·조우·런타임·서버·보상 실패 테스트를 작성한다**

```ts
expect(UNEXPLORED_POOL_BY_ID.mana_barrier.releaseStage).toBe("expanded");
expect(UNEXPLORED_POOL_BY_ID.mana_barrier.activeMonsters.map(({ id }) => id)).toEqual([
  "barrier_guardian",
  "rune_executor",
  "seal_watcher",
]);

expect(pickManaMonster(0)?.monsterId).toBe("barrier_guardian");
expect(pickManaMonster(0.34)?.monsterId).toBe("rune_executor");
expect(pickManaMonster(0.67)?.monsterId).toBe("seal_watcher");
```

런타임 테스트는 활성 특수 몬스터 16종과 전체 21종을 기대한다. `rune_executor`는 `atkType: "magic"`과 `mob_arcane_bolt`, `seal_watcher`는 `statusDamageReductionPct: 40`과 `{ kind: "brace", damageReduction: 6 }`을 기대한다. 집중 상태에서는 두 몬스터의 마법 방어가 증가하고 상태 피해 저항이 각각 10과 50이 되어야 한다.

서버 테스트는 마력 방벽체 풀 선택 후 몬스터 RNG `0.34`, `0.67`이 각각 `rune_executor`, `seal_watcher`로 보존되는지 확인한다. 기존 미출시 거부 테스트는 `regenerating_swarm/devouring_regenerator`로 대상을 옮긴다. 보상 테스트는 세 활성 몬스터 모두 다음 규칙을 기대한다.
...
- [x] **Step 2: 관련 테스트를 실행해 마력 방벽체가 launch 상태라 실패함을 확인한다**
...
Expected: 룬 집행자·봉인 감시체가 활성 목록과 런타임에 없거나 `not active` 오류가 발생해 FAIL

- [x] **Step 3: 마력 방벽체 출시 단계와 이미지 경로를 추가한다**

`src/adventure/data/v2/unexploredMonsterPools.ts`의 마력 방벽체 seed에 다음 값을 추가한다.
...
```ts
rune_executor: "/images/monster/v2/unexplored-rune-executor.webp",
seal_watcher: "/images/monster/v2/unexplored-seal-watcher.webp",
```
...
git commit -m "feat: activate mana barrier monster roster"
```

### Task 2: 룬 집행자와 봉인 감시체 이미지

**Files:**
- Create: `public/images/monster/v2/unexplored-rune-executor.webp`
- Create: `public/images/monster/v2/unexplored-seal-watcher.webp`

**Interfaces:**
- Consumes: Task 1의 `SPECIAL_IMAGE_PATHS`
- Produces: 512×512, 4채널, 알파 포함 WebP 두 장

- [x] **Step 1: 기존 결계 수호체를 스타일 참조로 룬 집행자를 생성한다**

Built-in image generation prompt: 정사각형 게임 몬스터 렌더, 실제 투명 배경, 방패 없는 날렵한 상아색 석재 골렘, 금색 테두리, 손·팔에 집중된 청색 공격 룬과 마력탄, 전신과 손 모두 표시, 결계 수호체의 재질·색·렌더 스타일 유지, 텍스트·로고·워터마크·배경 장면 없음.

- [x] **Step 2: 기존 결계 수호체를 스타일 참조로 봉인 감시체를 생성한다**

Built-in image generation prompt: 정사각형 게임 몬스터 렌더, 실제 투명 배경, 넓고 무거운 상아색 석재 골렘, 금색 테두리, 몸 주위의 청색 봉인 고리와 판석, 전신 표시, 결계 수호체의 재질·색·렌더 스타일 유지, 손방패·무기·텍스트·로고·워터마크·배경 장면 없음.
...
```bash
node -e "const sharp=require('sharp'); Promise.all(['public/images/monster/v2/unexplored-rune-executor.webp','public/images/monster/v2/unexplored-seal-watcher.webp'].map(async p=>[p,await sharp(p).metadata()])).then(x=>console.log(JSON.stringify(x,null,2)))"
...
```bash
git add public/images/monster/v2/unexplored-rune-executor.webp public/images/monster/v2/unexplored-seal-watcher.webp
git commit -m "feat: add mana barrier variant artwork"
```
