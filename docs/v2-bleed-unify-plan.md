...
- **피해 모델 차별화 채택**: 출혈 = ATK×계수 + flat(내 공격력 비례), 중독 = %최대HP(타깃 비례). 이름만 분리가 아니라 메커니즘을 다르게.
- **보스 안전장치 = A안 확정**: 중독 스택당 `min(최대HP×%, ATK×k)` — %HP 과녹임을 내 공격력 기준으로 묶는 절대 상한. 백업 다이얼 = 보스 DoT 배율(`isBoss`×n).
...
- tag 3종으로 분리:
  ...
  - **burn**: 스킬 연소(비전노바 등)
- **피해 모델 (확정)**:
  - 출혈(bleed) = **ATK × 계수 + flat** — 정액에서 공격력 비례로 전환(고렙에서 안 죽게). 계수는 작게(스택당 ATK의 ~0.1~0.15) — 10스택 × DEF무시 폭주 방지. (계수 실값은 sim 캘리브 다이얼.)
  - 중독(poison) = **%최대HP** (휴면 venom 배관 재활용) + **A안 절대 상한 확정**: 스택당 `min(최대HP×%, ATK×k)`. 보스 과녹임을 타깃 HP가 아니라 내 공격력 기준으로 제한 + 진행도 자동 스케일. 백업 다이얼=보스 DoT 배율(`isBoss`×n). (%·k 실값은 sim 다이얼.)
