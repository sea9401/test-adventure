...
## 2026-08-09 새 방어 체계 재보정

물리 방어 점감식, 적중도 대결형 회피 경감, 마력 장벽이 적용된 실제 협동 공격 경로를
기준으로 보스 6종을 다시 보정했다. `anchorDepth`, 발악 단계, 소환·공격 비용, 쿨다운,
보상 임계는 유지한다. 깊이 42 호수 보스는 기본 ATK를 최저 수준으로 낮춰도 한기 고정
피해만으로 목표 HP를 넘었기 때문에 한기 구조는 유지하고 스택당 피해만 35에서 17로
완화했다.

| 보스 | 공유 HP | 풀 HP 파생 ATK | 200회 중앙 생존율 | 중앙 기여율 |
|---|---:|---:|---:|---:|
| 산군 | 30,000 | 124 | 97.5% | 4.56% |
| 스콜피온 킹 | 80,000 | 288 | 91.5% | 7.29% |
| 호수의 괴물 | 270,000 | 80 | 87.0% | 7.02% |
| 공허의 대사제 | 630,000 | 456 | 76.5% | 7.00% |
| 흉포한 산군 | 1,200,000 | 595 | 66.5% | 3.65% |
| 심연어룡 | 1,400,000 | 966 | 100.0%* | 3.69% |

\* 심연어룡은 파생 ATK와 적중이 반올림 경계를 넘을 때 계보별 생존율이 이산적으로 변한다.
따라서 이진 생존율 상한 대신 중앙 계보의 잔여 HP 5% 이하를 검사한다. 확정 표본에서는
중앙 계보가 HP 약 4.93%를 남긴다. 하드 대응축은 흉포한 산군 VIT·LUK, 심연어룡 VIT·SPI다.
...
운영 상위 20명 익명·읽기 전용 교차 점검:

```bash
node --env-file=/run/adventure-rpg/production.env --env-file=.env.production \
  --import tsx scripts/sim-live-top-combat.ts --coop-only
```

운영 출력에는 보스별 생존율과 기여율 최소·중앙·95백분위만 포함한다. 이메일, 닉네임,
사용자 ID, 순위, 직업, 전투력, 개별 장비는 출력하거나 저장하지 않는다. 이 결과는 교차
검증용이며 자동 합격선은 고정 시드 일곱 성장 계보가 담당한다.

2026-08-09 운영 상위 20명 교차 검증(각 20회)의 익명 집계는 다음과 같다.

| 보스 | 생존율 | 기여율 최소 | 중앙 | p95 |
|---|---:|---:|---:|---:|
| 산군 | 100.0% | 100.00% | 100.00% | 100.00% |
| 스콜피온 킹 | 100.0% | 44.07% | 100.00% | 100.00% |
| 호수의 괴물 | 100.0% | 11.28% | 35.98% | 78.61% |
| 공허의 대사제 | 99.8% | 5.29% | 17.33% | 31.07% |
| 흉포한 산군 | 85.0% | 2.38% | 7.70% | 15.68% |
| 심연어룡 | 85.0% | 1.55% | 7.24% | 13.86% |

상위 20명은 일반 보스 성장 구간을 크게 넘어선 표본이므로 이 결과로 합성 합격선을
자동 변경하지 않았다. 다만 일반 4종의 높은 기여율은 엔드 이용자용 상위 보스 또는 별도
공유 HP 사다리를 설계할 때 우선 검토할 운영 근거로 남긴다.
