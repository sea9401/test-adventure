...
> **직업 스킬 연결 구현 메모 (LIVE)**
> - 새 jobId 별 접근 가능 스킬셋 = `v2SkillsByJob.ts:V2_SKILLS_BY_JOB`(공용[직군] + 생존 계파 시그니처 + **흡수한 사라진 계파 스킬**). 사라진 4계파의 12스킬(검투사 출혈·연환 콤보·워메이지 연사·독사 중독)을 흡수 직업(견습기사·권사·술사·자객)에 편입 — 고아 스킬 재활용 + 정체성 강화.
> - `classes.ts:elementalSkillsForClass` 가 flag on 일 때 `skillsForJob(jobIdFromLegacy(c, spec))` 로 분기 → 학습 풀·로드아웃 체인·전직 reconcile·상태 목록 전 호출처에 동일 적용.
> - **차수 게이팅 제거**(flag on): 옛 로직은 tier 인자로 시그니처를 차수당 1개씩 풀었는데, 새 시스템은 직업 tier=1(차수 폐지)이라 시그니처가 **전부 차단**되던 버그가 있었음 → 직업에 들어오면 그 직업 스킬 전부 학습 대상(SP 예산이 장착 상한). flag off = 기존 차수 로직 불변.
