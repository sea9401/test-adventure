...
| **PR-4** | 직업 정체성(사제 회복 패시브) + 매뉴얼 | 저 | ✅ 구현. acolyte_mana→회복강화·매뉴얼·sustain 게이트 통과 |
...
**PR-4 구현(2026-06-18)**: 사제(acolyte) = 힐러 정체성 완성. 패시브 `v2c_acolyte_mana` 를 마나(maxMpPct12)→**회복(healPowerPct 20)** 으로 리스킨(id 유지=세이브 호환·기존 리스킨 패턴). 사제 치유(active, pctLostHp)+회복(passive)+정신 비례 healMult(PR-1)=힐러. 매뉴얼 stats.tsx 갱신(정신=지원·대항 축·회복 주력·마법/치명형 몹 카운터·구 "쉬는 축" 노트 폐기·치명저항 cap 50 표기). cultivateProfile/jobBonus 무변경(사제 이미 spi 최다 anchor: cultivate spi2·jobBonus spi5). **🔑Codex 게이트=sustain sim(sim-v2-spi-sustain.ts) 통과**: 고-SPI 사제(치유+회복25)도 솔로 무적화 **없음** — 과도심도(d16~28) 전부 0% 승(힐이 데미지 못 앞지름), 오히려 sustain 빌드는 솔로 약함(공격 희생→killcap 미달). 회복25 = 무패시브와 동일 승률(무적화 0). 힐 가치는 협동보스/전쟁/장기전 niche. 테스트 v2Skills.test(사제 회복 패시브)+v2SkillsByJob.test 갱신. **SPI 아크: 지원(PR-1·4)+대항(PR-3a·b) 전부 LIVE → 죽은축 부활 완료**(잔여=PR-2 spi gear, 유저가 아이템 정리 후).
