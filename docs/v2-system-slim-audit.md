...
1차/2차 정리 이후 3차로 검토할 항목:

- API route 정리: v2 API의 인증, 세이브 로드, 응답 포맷, 에러 처리 패턴을 공통화한다.
- 데이터 검증 강화: 장비, 드랍, 보스, 직업, 칭호 데이터가 잘못 추가되면 테스트에서 바로 잡히도록 검증 범위를 넓힌다.
- 운영 점검 스크립트: 드랍률, 장비 위력, 세트 옵션, 칭호 조건을 한 번에 리포트로 뽑아 밸런스 확인 비용을 줄인다.
- 프론트 상태 동기화 정리: 골드, 숙련도, 전투/마을 주요 상태 갱신 흐름을 일관화해 새로고침 의존 버그를 줄인다.
