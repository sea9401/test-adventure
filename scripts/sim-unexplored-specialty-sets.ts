...
} from "../src/adventure/v2/combat/engine-pvp";
import { advanceTurnPvP } from "../src/adventure/v2/combat/engine.pvpPhase";
import {
  tickV2Dots,
  type V2Dot,
} from "../src/adventure/v2/combat/combatShared";
...
export type SimulationMetrics = {
  ...
  /** 실제 발동을 본표에서 귀속할 수 없으면 null이며 paired probe가 대신 검증한다. */
...
export type PairedEffectCheck = {
  ...
  resultDelta: number;
  passed: boolean;
  coverageDeltas?: Record<string, number>;
};
...
export type DefenseStress = {
  ...
  enemyCritChancePct: number;
  enemyCritMult: number;
  baselineCritResistPct: number;
  ironCritResistPct: number;
  baselineCriticalHits: number;
  ironCriticalHits: number;
...
function activationCount(
  ...
  if (scenario === "existing_weapon_3_plus_battle_revenge_3") {
    // 구조화 로그만으로 pending 소비와 20% 피해 귀속을 정확히 복원할 수 없어 본표에는
    // 큰 피격 기회만 남기고 실제 발동은 동일 seed paired probe로 검증한다.
    return null;
  }
...
const EXPECTED_DOT_COVERAGE_PATHS = [
  "pve_skill_poison",
  "pve_skill_bleed",
  "pve_skill_burn",
...
type DotProbeMode = "pve" | "pvp_p1" | "pvp_p2";

function runDotSkillProbe(
  player: PlayerCombat,
  skillId: V2SkillId,
  mode: DotProbeMode,
): readonly V2Dot[] {
  const actor = {
    ...player,
    hp: player.maxHp,
    mp: Math.max(INITIAL_PAID_SKILL_MP, player.maxMp ?? 0),
  ...
  const state = initialBattleStatePvP(
    side === "p1" ? actor : defender,
    side === "p2" ? actor : defender,
    "P1",
    "P2",
    side === "p1" ? skills : undefined,
    side === "p2" ? skills : undefined,
    undefined,
    undefined,
    side,
  );
  const result = castV2SkillOnAttackerTurnPvP(state, side).state;
  return side === "p1" ? result.p2.v2Dots : result.p1.v2Dots;
}

function runDotEquipmentProbe(
  player: PlayerCombat,
  mode: DotProbeMode,
): readonly V2Dot[] {
  const actor: PlayerCombat = {
    ...
    equipSignatures: [
      {
        trigger: "on_hit",
        label: "DOT probe poison",
        poisonChancePct: 100,
        poisonStacks: 1,
      },
      {
        trigger: "on_hit",
        label: "DOT probe bleed",
        bleedChancePct: 100,
        bleedStacks: 1,
      },
    ],
...
function dotTickDamage(
  dots: readonly V2Dot[],
  tag: V2Dot["tag"],
): number {
  const dot = dots.find((candidate) => candidate.tag === tag);
  return dot ? tickV2Dots([dot], 1_000_000).totalDmg : 0;
}

function runDotCoverage(
  effectPlayer: PlayerCombat,
  controlPlayer: PlayerCombat,
): Record<string, number> {
  const coverage: Record<string, number> = {};
  for (const mode of ["pve", "pvp_p1", "pvp_p2"] as const) {
    for (const [skillId, tag] of DOT_PROBE_SKILLS) {
      Math.random = () => 0;
      const effect = runDotSkillProbe(effectPlayer, skillId, mode);
      Math.random = () => 0;
      const control = runDotSkillProbe(controlPlayer, skillId, mode);
      coverage[`${mode}_skill_${tag}`] =
        dotTickDamage(effect, tag) - dotTickDamage(control, tag);
    }
    Math.random = () => 0;
    const effect = runDotEquipmentProbe(effectPlayer, mode);
    Math.random = () => 0;
    const control = runDotEquipmentProbe(controlPlayer, mode);
    for (const tag of ["poison", "bleed"] as const) {
      coverage[`${mode}_equipment_${tag}`] =
        dotTickDamage(effect, tag) - dotTickDamage(control, tag);
    }
  }
  return coverage;
...
function runPairedEffectChecks(
  ...
  try {
    ...
    const dotCoverage = runDotCoverage(triad, triadControl);
...
function runDefenseStress(
  ...
  const run = (scenario: "existing_t6_baseline" | "iron_line_3") => {
    ...
    try {
      for (let trial = 0; trial < seedCount; trial += 1) {
        ...
        criticalHits += result.finalState.log.filter(
          (entry) =>
            entry.kind === "enemy_attack" && entry.text.includes("[치명타]"),
        ).length;
        survived.push(
  ...
  return {
    definition:
      "동일 고HP·고방어 적의 동일 공격 압력(치명 30%, 배율 1.5)에서 사망 또는 상한까지 생존한 적 행동 수(장비 포함 치명 저항 적용, 플레이어 선처치 불가)",
    actionLimit,
    ...
    enemyCritChancePct: stressEnemy.critPct ?? 0,
    enemyCritMult: stressEnemy.critMult ?? 1.5,
    baselineCritResistPct: baseline.critResistPct,
    ironCritResistPct: iron.critResistPct,
    baselineCriticalHits: baseline.criticalHits,
    ironCriticalHits: iron.criticalHits,
    survivalUpliftPct:
  ...
  try {
    for (const scenario of SCENARIO_IDS) {
      ...
      for (let trial = 0; trial < seedCount; trial += 1) {
        ...
        const observedActivations = activationCount(
          scenario,
          player,
          log,
          result.finalState,
        ...
        if (observedActivations === null) activations = null;
        else if (activations !== null) activations += observedActivations;
      }
...
export function validateUnexploredSpecialtySetSimulation(
  ...
  for (const row of report.rows) {
    ...
    if (
      row.metrics.activationCount !== null &&
      row.metrics.activationCount <= 0
    ) {
  ...
  const dotCoverage =
    report.pairedEffectChecks.triad_dot_amplification.coverageDeltas;
  if (
    !dotCoverage ||
    Object.keys(dotCoverage).length !== EXPECTED_DOT_COVERAGE_PATHS.length ||
    EXPECTED_DOT_COVERAGE_PATHS.some(
      (path) =>
        !Number.isFinite(dotCoverage[path]) || (dotCoverage[path] ?? 0) <= 0,
    )
  ) {
    errors.push("triad_dot_amplification: DOT 실제 엔진 경로 누락");
  }
  if (
  ...
  if (
    ...
    report.defenseStress.enemyCritChancePct <= 0 ||
    report.defenseStress.enemyCritMult <= 1 ||
    (report.seedCount >= 100 &&
      (report.defenseStress.baselineCriticalHits <= 0 ||
        report.defenseStress.ironCriticalHits >=
...
function printReport(report: UnexploredSpecialtySimulationReport): void {
  ...
  console.table(
    report.rows.map((row) => {
      ...
      return {
        ...
        "관측 발동": row.metrics.activationCount ?? "paired probe",
  ...
  console.table({
    existing_t6_baseline: {
      ...
      "치명 저항%": report.defenseStress.baselineCritResistPct,
      "관측 치명타": report.defenseStress.baselineCriticalHits,
    ...
    iron_line_3: {
      ...
      "치명 저항%": report.defenseStress.ironCritResistPct,
      "관측 치명타": report.defenseStress.ironCriticalHits,
  ...
  console.log("삼재 DOT 실제 엔진 경로별 동일 조건 효과 on/off 틱 피해 차이");
  console.table(
    Object.entries(
      report.pairedEffectChecks.triad_dot_amplification.coverageDeltas ?? {},
    ).map(([path, damageDelta]) => ({ path, damageDelta })),
  );
}
