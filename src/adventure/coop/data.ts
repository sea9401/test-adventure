...
// 보스 콘텐츠 데이터는 v2 재설계 대기 — 시스템(타입·리스폰·보상티어·cron)은 보존하고
// 정의만 비워둔다. v2 협동/월드 보스를 여기에 RegionId 별로 다시 채우면 됨.
// 빈 맵이면 respawnCoopRegion 이 모든 region 에서 def=undefined 로 no-op (cron 무해).
// (구 v1 보스 dragon_nest 「태고의 노룡」 / forgotten_seal 「별을 잊은 것」 제거 — git 이력 보존.)
