...
// 봉납(별빛 조각/골드) 누계가 임계 이상이면 마스터가 회관 등급을 ★1~★5 까지 승급.
// 전투력·자원율 등 어떤 능력 수치에도 영향이 없는 게 핵심 — 무소속 패널티 회피.
// 설계 보고서: docs/guild-lodge-plan.md
//
// ⚠️ 명명 주의 — `stardust` / `stardustTotal` / kind="stardust" 식별자는 의미상
// 「별빛 조각」(materials.starfall_shard) 을 가리킨다. 초기엔 materials.stardust
// (=별먼지) 를 차감했으나 4막 메인 자원과 분리가 안 돼서, 회관만 5막
// 엔드게임 재료인 별빛 조각으로 갈아치웠다. DB 컬럼/enum 값 유지로 마이그레이션
// 부담 0 — 의미만 바뀜. 서버 debitDonation 이 mats.starfall_shard 를 차감.
...
// 임계는 양쪽 통화 동시 만족이어야 승급 — 별빛 조각 단독·골드 단독 누적은 부족.
// 두 통화 같이 흘러가는 사회적 협업을 자연스럽게 강제. ★4→★5 사이 4x 점프로
// 활동 길드와 한가한 길드의 격차 가시화. 별빛 조각으로 전환하며 임계를 1/5
// 로 낮춤 — 별빛 조각은 5막 별빛 변종 전용 희귀 자원이라 별먼지 시절 페이스
// 대비 수급이 훨씬 느림 (운영 데이터 보고 조정 예정).
export const LODGE_RANK_THRESHOLD = {
  1: { stardust: 0, gold: 0 },
  2: { stardust: 40, gold: 5000 },
  3: { stardust: 160, gold: 20000 },
  4: { stardust: 480, gold: 60000 },
  5: { stardust: 1280, gold: 150000 },
