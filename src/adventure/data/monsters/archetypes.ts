...
//   회피/치명/마법형(matk) + caster 의 마법 스킬 시전(v2Skills) 기본값.
...
const ARCHETYPE_PROFILE: Record<Archetype, Partial<Monster>> = {
  ...
  // 마법형 — 평타도 마법(atkType)·기본 마법 스킬 시전(엔진 배선됨). 몹이 v2Skills 직접 지정 시 우선.
  caster: {
    atkType: "magic",
    v2Skills: { learned: ["mob_mana_bolt"], equipped: ["mob_mana_bolt"] },
    v2MaxMp: 80,
  },
};
...
export function resolveMonsterArchetype(m: Monster): Monster {
  ...
  if (p.v2Skills != null && out.v2Skills == null) out.v2Skills = p.v2Skills;
  if (p.v2MaxMp != null && out.v2MaxMp == null) out.v2MaxMp = p.v2MaxMp;
