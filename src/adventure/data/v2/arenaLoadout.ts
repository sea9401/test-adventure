// 아레나 세팅(로드아웃) — 캐릭터의 "현재 빌드" 스냅샷: 장착 스킬 + 활성 전투 패턴(겜빗) +
// 장착 장비(슬롯→개체 iid). 저장해 두고 선택하면 그 빌드로 현재 캐릭터 장착을 즉시 교체한다.
// 라이브 전투(사냥·아레나)가 모두 이 장착 상태를 읽으므로 캐릭터 전역에 적용된다(아레나 전용 아님).
//
