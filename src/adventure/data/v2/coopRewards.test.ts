...
describe("coopRewards", () => {
  ...
  it("rollCoopEquipmentBoxItem — 일반 상자는 티어 범위, 전용 상자는 고정 풀에서 반환", () => {
    for (const boss of COOP_BOSS_KIND_IDS) {
      const got = rollCoopEquipmentBoxItem(boss, () => 0);
      expect(got).toBeTruthy();
      const item = V2_EQUIPMENT[got!];
      if (COOP_EQUIPMENT_BOX[boss].fixedItemIds) {
        expect(COOP_EQUIPMENT_BOX[boss].fixedItemIds).toContain(got);
        expect(item.setTags).toContain("abyssal_current");
