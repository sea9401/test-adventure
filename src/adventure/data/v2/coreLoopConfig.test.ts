...
describe("calcSpBudget — 스킬포인트 예산 (직업 해금 수집)", () => {
  it("기본 SP 는 40이고 숙련도 groups 는 예산에 직접 영향을 주지 않는다", () => {
    ...
    expect(calcSpBudget(null)).toBe(40);
  ...
  it("소프트캡 없이 도감/열매 보너스를 그대로 더한다", () => {
    expect(calcSpBudget({}, 0, 2, 4)).toBe(46);
  ...
  it("손상 입력 방어", () => {
    expect(calcSpBudget({ a: { cumLevel: NaN as unknown as number } })).toBe(40);
    expect(calcSpBudget({}, 0, 0, NaN)).toBe(40);
  });
});
