...
describe("monster craft material drops", () => {
  ...
  it("drops the venom gland only from a defeated cave spider", () => {
    expect(rollMonsterCraftMaterialDrops("동굴 거미", () => 0.0199)).toEqual({
      [MONSTER_CRAFT_MATERIAL_ID.caveSpiderVenomGland]: 1,
    });
    expect(rollMonsterCraftMaterialDrops("동굴 거미", () => 0.02)).toEqual(
      {},
    );
  });
  ...
  it("does not consume RNG for monsters without a dedicated material", () => {
    let draws = 0;
    expect(
      rollMonsterCraftMaterialDrops("암반 골렘", () => {
        draws += 1;
        return 0;
      }),
    ).toEqual({});
