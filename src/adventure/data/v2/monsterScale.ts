...
//   - FLOOR5_MULT: hp/atk/def 를 축소 — 변동성(작은 몹 vs 거인) 보존하며 v2 범위로.
//   - FLOOR5_DEF_CAP: def 는 배율 후에도 절벽이라 추가 cap — 관통 0 인 v2 가 깰 수 있게.
//   - exp 는 난이도 배율과 분리 — 레벨 밴드 기준 base(×1.0). Lv70~100 은 exp 곡선이 가팔라
//     exp 가 더 필요한 구간인데, 난이도(×0.4)에 묶으면 leveling 이 punishing 해진다
//     (floor 5 ×0.4 exp 71/킬 → Lv75→100 ~47일). floor 1~4 와 동일하게 base exp 사용.
// floor 1~4 는 이미 적정(불꽃 골렘 def26·hp680)이라 손대지 않고, floor 6~8 엔드 파밍은
// 의도적으로 어려워야 하므로 미적용(별빛 ×1.2~4.0 그대로).
// sim-v2-progression --skills + sim-v2-exp-pacing 캘리브 후 다이얼링.
...
export function scaleMonsterForFloor(
  ...
  // 난이도 배율(hp/atk/def 용) — floor 5 만 ×0.4.
  const statMult = FLOOR_DIFFICULTY[floor] * (floor === 5 ? FLOOR5_MULT : 1);
  const hp = Math.max(1, Math.round(monster.hp * statMult));
  const atk = Math.max(1, Math.round(monster.atk * statMult));
  // floor 5 는 배율 후 def 를 추가 cap (관통 0 인 v2 의 절벽 방지).
  const rawDef = Math.max(0, Math.round(monster.def * statMult));
  const def = floor === 5 ? Math.min(rawDef, FLOOR5_DEF_CAP) : rawDef;
  // exp 는 난이도 배율과 분리 — FLOOR_DIFFICULTY 만 적용(floor 5 = ×1.0 base, 6~8 = ×1.2~4.0).
  const exp = Math.max(0, Math.round(monster.exp * FLOOR_DIFFICULTY[floor]));
