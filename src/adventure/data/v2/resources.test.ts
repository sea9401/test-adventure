...
describe("자원 산출 — computeStoneYield", () => {
  ...
  it("0 시간 → 0 stone", () => {
    expect(computeStoneYield(1, "mine", now, now).gained).toBe(0);
  });

  it("1 시간 → tier 산출량 그대로 (mine)", () => {
    for (const tier of [1, 2, 3, 4] as const) {
      expect(computeStoneYield(tier, "mine", now - HOUR, now).gained).toBe(
  ...
  it("4 시간 = 4 × 산출량 (mine)", () => {
    expect(computeStoneYield(2, "mine", now - 4 * HOUR, now).gained).toBe(
      4 * STONE_YIELD_PER_HOUR[2],
    );
  });

  it("offline cap — 24+ 시간 흘러도 24시간치만", () => {
    const r = computeStoneYield(4, "mine", now - 100 * HOUR, now);
  ...
  it("정확히 24h 경계 — cap 그대로", () => {
    const r = computeStoneYield(
      2,
      "mine",
      now - HARVEST_OFFLINE_CAP_HOURS * HOUR,
      now,
    );
  ...
  it("미래 시점 lastHarvested — 0 (음수 elapsed 보호)", () => {
    expect(computeStoneYield(3, "mine", now + HOUR, now).gained).toBe(0);
  });

  it("village — mine 의 50% 산출", () => {
    for (const tier of [1, 2, 3, 4] as const) {
      expect(computeStoneYield(tier, "village", now - HOUR, now).gained).toBe(
        Math.floor(STONE_YIELD_PER_HOUR[tier] * 0.5),
      );
    }
  });

  it("tower / fort — stone 산출 X", () => {
    expect(computeStoneYield(3, "tower", now - 4 * HOUR, now).gained).toBe(0);
    expect(computeStoneYield(3, "fort", now - 4 * HOUR, now).gained).toBe(0);
  });
});
