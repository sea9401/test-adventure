...
describe("SP_FRUIT config 일관성", () => {
  ...
  it("사용 캡 합 = 11 (I 3·II 3·III 5 → 최대 +11 SP)", () => {
...
describe("parseSpFruitUsed", () => {
  it("기본 0 (빈/불량 입력)", () => {
    expect(parseSpFruitUsed(undefined)).toEqual({ 1: 0, 2: 0, 3: 0 });
    expect(parseSpFruitUsed(null)).toEqual({ 1: 0, 2: 0, 3: 0 });
    expect(parseSpFruitUsed("nope")).toEqual({ 1: 0, 2: 0, 3: 0 });
    expect(parseSpFruitUsed({ 1: "x", 2: NaN })).toEqual({ 1: 0, 2: 0, 3: 0 });
  });

  it("정상 입력 보존", () => {
    expect(parseSpFruitUsed({ 1: 2, 2: 1, 3: 4 })).toEqual({ 1: 2, 2: 1, 3: 4 });
  });

  it("캡 초과·음수·소수 클램프", () => {
    expect(parseSpFruitUsed({ 1: 99, 2: -5, 3: 4.9 })).toEqual({
...
describe("spCapBonusFromFruits / spCapBonusFromRaw", () => {
  it("없음/null = 0", () => {
    ...
    expect(spCapBonusFromFruits({ 1: 0, 2: 0, 3: 0 })).toBe(0);
  });

  it("부분 사용 합산", () => {
    expect(spCapBonusFromFruits({ 1: 2, 2: 0, 3: 1 })).toBe(3);
  });

  it("전부 캡 = 최대 11", () => {
    expect(spCapBonusFromFruits({ 1: 3, 2: 3, 3: 5 })).toBe(11);
  });

  it("캡 초과 입력도 11 로 클램프", () => {
    expect(spCapBonusFromFruits({ 1: 99, 2: 99, 3: 99 })).toBe(11);
  });
...
describe("fruitTierForBoss / fruitTierForMaterial", () => {
  it("보스 kind → 등급", () => {
    ...
    expect(fruitTierForBoss("abyssal_tyrant")).toBeNull();
...
describe("calcSpBudget spCapBonus 배선 (byte-identical 가드)", () => {
  ...
  it("보너스는 캡 위에 그대로 더해진다", () => {
    ...
    expect(calcSpBudget(groups, 11)).toBe(base + 11);
  });
  ...
  it("실제 SP 열매 사용분 → 예산 증가", () => {
    const base = calcSpBudget(groups);
    const used: Record<SpFruitTier, number> = { 1: 3, 2: 3, 3: 5 };
    expect(calcSpBudget(groups, spCapBonusFromFruits(used))).toBe(base + 11);
  });
});
