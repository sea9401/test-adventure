...
//   사용 캡). docs: 협동 보스 리워크. 보스별 등급(I/II/III)·캡 도달 시 사용 차단(거래만).
...
// 등급 → 정의. 사용 캡 I=3·II=3·III=5 → 캐릭터당 최대 +11 SP(오너 확정).
...
export function parseSpFruitUsed(raw: unknown): SpFruitUsed {
  ...
  const out: SpFruitUsed = { 1: 0, 2: 0, 3: 0 };
...
// 사용 횟수 → SP 최대치 보너스(각 등급 캡 클램프 후 × spPerUse 합). 미장착/없음=0. 최대 11.
