...
// 초반 외 무의미했던 체감 보정. → 여기 값은 이제 **레벨 1 기준(앵커)** 이다.
// 2026-06-23 진행도 연동: 실효 시급 = 기준시급 × tileYieldLevelMult(소유자 레벨). L1=현행 유지,
// L100 ~3×. 사냥 수입이 깊이로 스케일하는 것과 같은 취지 — settlement 은 secondary 라 완만.
export const TILE_YIELD_PER_HOUR: Record<TileSettlementTier, number> = {
  frontier: 0, // 땅 미보유 = 생산 없음 (레벨 무관)
  village: 300, // L1 기준 — 실효는 ×레벨배율
  city: 750,
  metropolis: 1600,
};
export const TILE_YIELD_CAP_HOURS = 24; // 누적 상한(시간) — 1일 1회 풀수확 리듬.

// 진행도(레벨) 연동 시급 배율 — L1 1.0× → L_cap 까지 선형 증가(slope×(cap-1) 만큼).
// LEVEL_CAP 은 V2_LEVEL_CAP(100) 미러 — tileConfig 무import 유지 위해 로컬 상수(레벨캡 변경 시 동기).
// ⚠️ 레벨은 재전직(환생) 시 1로 리셋된다 → 환생 직후 시급 일시 하락(재성장으로 회복). 의도된 트레이드오프.
export const TILE_YIELD_LEVEL_CAP = 100;
export const TILE_YIELD_LEVEL_SLOPE = 0.02; // 레벨당 +배율 (L1 1.0 → L100 ~2.98×). 다이얼.
export const tileYieldLevelMult = (level: number): number => {
  const L = Math.max(1, Math.min(TILE_YIELD_LEVEL_CAP, Math.floor(level || 1)));
  return 1 + (L - 1) * TILE_YIELD_LEVEL_SLOPE;
};

// 보류 수확량(골드) — 경과시간 × 시급(레벨 배율 적용), 캡 시간으로 상한. 서버·클라 공통(epoch ms).
// level 미지정(기본 1)이면 배율 1.0 = 기준시급(하위호환).
export const tilePendingYield = (
  ...
  const perHour = (TILE_YIELD_PER_HOUR[tier] ?? 0) * tileYieldLevelMult(level);
