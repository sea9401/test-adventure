...
function slotConceptLine(
  ...
  return v2EquipmentBySlot(slot)
    .filter(
      (i) =>
        i.concept === concept && !isUnique(i) && !i.craftOnly && !i.bossOnly,
    ) // 그리드는 정규만(유니크·제작전용·보스전용 제외)
    .sort((a, b) => a.tier - b.tier)
    .map((i) => i.tier);
}

describe("V2_EQUIPMENT grid (55종 — 6슬롯)", () => {
  it("정규 그리드 55종 + 유니크 6 + 제작전용 7 + 보스전용 2 (그리드 밖)", () => {
    const all = Object.values(V2_EQUIPMENT);
    expect(
      all.filter((i) => !isUnique(i) && !i.craftOnly && !i.bossOnly),
      "정규 그리드",
    ).toHaveLength(55);
    expect(all.filter((i) => isUnique(i)), "유니크").toHaveLength(6);
    expect(all.filter((i) => i.craftOnly), "제작전용").toHaveLength(7);
    expect(all.filter((i) => i.bossOnly), "보스전용").toHaveLength(2);
  });
