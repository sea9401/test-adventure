...
//   - **전 장비 위력 ×3 (2026-06-07)** — 루트 게임답게 "장비 = 공·방 주력". 전엔 STR/VIT(레벨)이
//     지배했으나 위력 상향으로 장비가 주력·스탯은 보조. 1차로 ×5 했다가 체감이 과해(무기≈74%·
//     갑옷≈68% 비중) 사용자 요청으로 ×3 으로 낮춤(중간 강도). god-roll 추격(편차 0.65)은 유지.
//     장갑/신발/반지/목걸이 위력은 소폭(정체성=옵션). 몹 스케일·편차 유지. 다이얼=×3(베이스 ×3 baked).
...
export const V2_EQUIPMENT: Record<V2EquipmentId, V2Equipment> = {
  ...
  v2_uniq_starcleaver: {
    ...
    power: 36, // 최고티어(T3) 정규(72~93)보다 낮은 위력 + 폭발 치명 = 사이드그레이드
    weight: 1,
    options: { crit: 5 },
    rarity: "unique",
  },
  ...
  v2_meadow_bow: {
    ...
    power: 27, // 무기위력 ×5 (craftOnly — E 단계서 정리 예정)
    weight: 1,
    options: { crit: 1 },
    craftOnly: true,
  },
  v2_spider_venom_dagger: {
    ...
    power: 33, // 무기위력 ×5 (craftOnly — E 단계서 정리 예정)
    weight: 1,
    options: { crit: 1 },
    craftOnly: true,
  },
  v2_wolffang_staff: {
    ...
    power: 45, // 무기위력 ×5 (craftOnly — E 단계서 정리 예정)
    weight: 1,
    options: { mp: 12 },
    craftOnly: true,
  },
