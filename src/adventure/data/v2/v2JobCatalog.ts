 ...
 *  - jobUnlocked: 특정 선행 직업이 이미 해금돼 있어야 함. 숙련도 숫자 없이 계보만 유지할 때 사용.
 */
 ...
 * 베이스 직군 숙련도로 게이트(예: 견습 병사 숙련도 1000 → 방패병/견습 기사). prereq 키 = tier-1 base id.
 ...
 *   자연스러워짐. 값 2500 ≈ 그 2차 직업 장기 루프. (옛 직군 게이팅 #925 supersede.)
 */
export const TIER3_UNLOCK_CUMLEVEL = 2500;
...
export const MINING_TIER2_UNLOCK_CUMLEVEL = 900;
...
export const MINING_LEVEL_REQUIREMENTS = {
  miningtechnician: 10,
  masterminer: 20,
  minemaster: 35,
  legendaryminer: 50,
 ...
 */
export const TIER5_UNLOCK_CUMLEVEL = 18000;
...
export const V2_JOB_CATALOG: Record<string, V2JobDefinition> = {
  ...
  miner: {
    ...
    jobBonus: { vit: 3, str: 2 },
    unlock: { prereqs: { survivor: MINING_TIER2_UNLOCK_CUMLEVEL } },
  },
  ...
  miningtechnician: {
    ...
    tier: 3,
    cultivateProfile: { vit: 2, str: 1, dex: 1 },
    jobBonus: { vit: 8, str: 7 },
    unlock: {
      prereqs: {},
      extraConditions: [
        { type: "jobUnlocked", jobId: "miner" },
        { type: "miningLevel", min: MINING_LEVEL_REQUIREMENTS.miningtechnician },
      ],
    },
  },
  ...
  bloodtemplar: {
    ...
    jobBonus: { str: 9, vit: 6, spi: 5 }, // 광전사+사제 — HP 소모·방벽·약화 탱딜
    unlock: {
      prereqs: {
  ...
  masterminer: {
    ...
    tier: 4,
    cultivateProfile: { vit: 2, str: 1, dex: 1 },
    jobBonus: { vit: 12, str: 10 },
    unlock: {
      prereqs: {},
      extraConditions: [
        { type: "jobUnlocked", jobId: "miningtechnician" },
        { type: "miningLevel", min: MINING_LEVEL_REQUIREMENTS.masterminer },
      ],
    },
  },
  crusader: {
  ...
  minemaster: {
    ...
    unlock: {
      prereqs: {},
      extraConditions: [
        { type: "jobUnlocked", jobId: "masterminer" },
        { type: "miningLevel", min: MINING_LEVEL_REQUIREMENTS.minemaster },
      ],
    },
  },
  transcendent: {
  ...
  legendaryminer: {
    ...
    unlock: {
      prereqs: {},
      extraConditions: [
        { type: "jobUnlocked", jobId: "minemaster" },
        { type: "miningLevel", min: MINING_LEVEL_REQUIREMENTS.legendaryminer },
      ],
    },
  },
  blooddemon: {
...
export const CATALOG_USES_MINING_LEVEL_CONDITION: boolean = V2_JOB_LIST.some(
  (job) =>
    (job.unlock.extraConditions ?? []).some((c) => c.type === "miningLevel"),
);
...
function isJobUnlockedInternal(
  ...
  for (const cond of job.unlock.extraConditions ?? []) {
    if (!extraConditionMet(cond, proficiency, ctx, seen)) return false;
  }
  return true;
}
...
export function isDirectNextJob(
  ...
  return (
    Object.prototype.hasOwnProperty.call(job.unlock.prereqs, currentJobId) ||
    (job.unlock.extraConditions ?? []).some(
...
const V2_MINING_JOB_IDS = new Set([
  "miner",
  "miningtechnician",
  "masterminer",
  "minemaster",
...
export function isMiningJobId(jobId: string): boolean {
