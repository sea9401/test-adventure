import type { Monster } from "@/adventure/data/monsters/types";

// v2 전용 몬스터 카탈로그 — v2 사냥터(dungeon hunt)·훈련장(spar)이 쓰는 몹을 V1 MONSTERS
// 에서 분리해 v2 가 직접 소유한다(콘텐츠 링 확장으로 종 수는 계속 늘어남 — 수치 주석 생략). 옛 구조는 v2 가 V1 region/quest 몹을 stat 출처로 빌려 써서,
// v2 몹 하나 고치려 해도 "이게 V1 퀘스트랑 공유되나?"를 매번 확인해야 했다(예: 산적 궁수가
// 옛 노상강도 공유). 이제 v2 는 여기만 보면 된다.
//
// 규칙:
//   - 스탯/스킬만 보유. 표시 이름·초상화·속성은 dungeon.ts 의 DungeonEnemy 가 덮어쓴다.
//     (훈련 더미만 dungeon 을 안 거치고 직접 조회 → name/image 를 여기서 직접 쓴다.)
//   - drops 는 의도적으로 비움. v2 드랍은 몬스터별이 아니라 floor 풀(dungeonDrops/Equip/Unique)
//     기반이라 hunt 가 monster.drops 를 읽지 않는다. 옛 V1 드랍(재료/레시피 id)을 복사하면
//     V1 카탈로그로 다시 묶이므로 일부러 제외.
//   - key 는 dungeon.ts FLOOR*_ENEMIES 의 key 및 spar 의 V2_SPAR_DUMMY_ID 와 일치해야 한다.
export const V2_MONSTERS: Record<string, Monster> = {
  // ── 들판 (floor 1) — 온보딩 풀. 스킬·상태이상 없음. ──────────────────
  들개: { name: "들개", tags: ["beast"], hp: 38, atk: 3, def: 1, spd: 4, exp: 3 },
  두더지: { name: "두더지", tags: ["beast"], hp: 27, atk: 2, def: 0, spd: 3, exp: 2 },
  박쥐: { name: "박쥐", tags: ["beast"], hp: 49, atk: 5, def: 2, spd: 7, exp: 4 },
  동굴뱀: { name: "동굴뱀", tags: ["beast"], hp: 57, atk: 6, def: 2, spd: 5, exp: 5 },
  거미: { name: "거미", tags: ["beast"], hp: 72, atk: 7, def: 3, spd: 6, exp: 6 },

  // ── 산적/짐승 (옛 "깊은 산" — 던전 테마는 #삭제, 길드 퀘스트·월드 존·제작 재료에서 계속 사용) ──
  "떠돌이 약탈자": {
    name: "떠돌이 약탈자",
    tags: ["humanoid"],
    hp: 280,
    atk: 24,
    def: 11,
    spd: 6,
    exp: 28,
    skill: { kind: "pierce", name: "급소 노리기", armorPierce: 3 },
  },
  "절벽 늑대": {
    name: "절벽 늑대",
    tags: ["beast"],
    hp: 240,
    atk: 22,
    def: 9,
    spd: 7,
    exp: 22,
  },
  "부서진 골렘": {
    name: "부서진 골렘",
    tags: ["golem"],
    hp: 180,
    atk: 13,
    def: 6,
    spd: 2,
    exp: 14,
  },
  들소: {
    name: "들소",
    tags: ["beast"],
    hp: 320,
    atk: 25,
    def: 14,
    spd: 4,
    exp: 30,
    skill: { kind: "heavy_blow", name: "들이받기", everyPhases: 3, multiplier: 1.5 },
  },

  // ── 프론티어 밴드 A — 마른 협곡 (깊이 7~12). 깊이당 배율↑(dungeonLadder). ──────
  모래도마뱀: {
    name: "모래도마뱀",
    tags: ["beast"],
    hp: 220,
    atk: 18,
    def: 9,
    spd: 6,
    exp: 20,
  },
  // 도적·포식자·우두머리는 치명형(SPI PR-3b) — 가끔 치명 버스트, 정신(치명저항)이 카운터.
  "협곡 도적": {
    name: "협곡 도적",
    tags: ["humanoid"],
    hp: 195,
    atk: 23,
    def: 6,
    spd: 7,
    exp: 22,
    critPct: 30,
  },
  "바위 골렘": {
    name: "바위 골렘",
    tags: ["golem"],
    hp: 330,
    atk: 16,
    def: 14,
    spd: 2,
    exp: 26,
    skill: { kind: "heavy_blow", name: "내리찍기", everyPhases: 3, multiplier: 1.5 },
    // v2 시그니처 액티브(MP 게이트) — 분쇄 일격. 정해진 MP(전투 내 재생 없음)로 시전 ≈3회 제한.
    v2Skills: { learned: ["mob_crushing_blow"], equipped: ["mob_crushing_blow"] },
    v2MaxMp: 90,
  },
  "회오리 매": {
    name: "회오리 매",
    tags: ["beast"],
    hp: 160,
    atk: 20,
    def: 5,
    spd: 9,
    exp: 21,
  },
  "스파크 전갈": {
    name: "스파크 전갈",
    tags: ["beast"],
    hp: 210,
    atk: 19,
    def: 8,
    spd: 6,
    exp: 22,
    skill: { kind: "pierce", name: "독침", armorPierce: 3 },
  },

  // ── 프론티어 밴드 B — 얼음 호수 (깊이 8~14). water·wind, 둔중·탱키. ──────
  "서리 늑대": { name: "서리 늑대", tags: ["beast"], hp: 250, atk: 20, def: 10, spd: 6, exp: 23 },
  "빙벽 골렘": {
    name: "빙벽 골렘",
    tags: ["golem"],
    hp: 360,
    atk: 15,
    def: 16,
    spd: 2,
    exp: 27,
    skill: { kind: "heavy_blow", name: "내리찍기", everyPhases: 3, multiplier: 1.5 },
  },
  // 정령·망령은 마법형(SPI PR-3a) — 마법방어(정신)로 경감, 물리탱크 약점. atk 는 물리몹 스케일 유지(총피해 중립).
  "얼음 정령": { name: "얼음 정령", tags: ["spirit"], hp: 200, atk: 22, def: 8, spd: 4, exp: 23, atkType: "magic" },
  "눈보라 매": { name: "눈보라 매", tags: ["beast"], hp: 170, atk: 21, def: 6, spd: 8, exp: 22 },
  "호수 망령": { name: "호수 망령", tags: ["undead"], hp: 240, atk: 19, def: 9, spd: 5, exp: 25, atkType: "magic" },

  // ── 프론티어 밴드 C — 심층 동굴 (깊이 15~21). earth·void, 기습·혼합. ──────
  // evasionPct 필드는 몬스터의 회피도(레거시 필드명)다. 후반 밴드(C→F)로 갈수록
  // 민첩 몬스터의 회피도가 높아져 직접 피해를 더 줄이고, 플레이어 적중도 투자가 이를 상쇄한다.
  // 골렘·중갑형은 0 또는 낮은 값이며 깊이별 자동 스케일 없이 개별 지정한다.
  "동굴 거미": { name: "동굴 거미", tags: ["beast"], hp: 175, atk: 21, def: 6, spd: 9, exp: 23, evasionPct: 10 },
  "암반 골렘": {
    name: "암반 골렘",
    tags: ["golem"],
    hp: 350,
    atk: 16,
    def: 16,
    spd: 2,
    exp: 28,
    skill: { kind: "heavy_blow", name: "내리찍기", everyPhases: 3, multiplier: 1.5 },
    v2Skills: { learned: ["mob_crushing_blow"], equipped: ["mob_crushing_blow"] },
    v2MaxMp: 90,
  },
  "박쥐 떼": { name: "박쥐 떼", tags: ["beast"], hp: 150, atk: 20, def: 4, spd: 9, exp: 21, evasionPct: 12 },
  "심연 벌레": {
    name: "심연 벌레",
    tags: ["beast"],
    hp: 250,
    atk: 20,
    def: 10,
    spd: 6,
    exp: 25,
    skill: { kind: "pierce", name: "파고들기", armorPierce: 4 },
  },
  "동굴 포식자": { name: "동굴 포식자", tags: ["beast"], hp: 260, atk: 24, def: 10, spd: 6, exp: 27, critPct: 30 },

  // ── 프론티어 밴드 D — 잊힌 성소 (깊이 22~28). starlight·earth, 마법 버스트. ──────
  "수호 석상": {
    name: "수호 석상",
    tags: ["golem"],
    // hp 340→289(−15%, 2026-06-22): 잊힌 성소(d25-30) 반물리 로스터(탱크2 hp~3300+heavy_blow·
    //   마법1·회피관통2)가 물리 빌드(STR/VIT/LUK) 풀승률을 ~60%로 떨궈, 두 탱크가 저DPS 물리에
    //   안 깨지는 벽. HP만 트림해 절벽 제거(STR86·VIT76-86·LUK91·d25-30 skills-ON)·테마 정체성
    //   (마법/회피 성소·물리/탱크 약간 불리 vs INT/DEX/BAL 100) 유지. 두 탱크는 d25-30 전용(스필오버 0).
    hp: 289,
    atk: 17,
    def: 16,
    spd: 3,
    exp: 28,
    skill: { kind: "heavy_blow", name: "내리찍기", everyPhases: 3, multiplier: 1.5 },
  },
  "성소 망령": { name: "성소 망령", tags: ["undead"], hp: 190, atk: 25, def: 6, spd: 6, exp: 25, evasionPct: 11, atkType: "magic" },
  "빛의 정령": {
    name: "빛의 정령",
    tags: ["spirit"],
    hp: 210,
    atk: 22,
    def: 8,
    spd: 6,
    exp: 25,
    evasionPct: 14,
    skill: { kind: "pierce", name: "신성 관통", armorPierce: 3 },
  },
  "타락한 사제": {
    name: "타락한 사제",
    tags: ["humanoid"],
    hp: 200,
    atk: 24,
    def: 7,
    spd: 6,
    exp: 26,
    evasionPct: 9,
    skill: { kind: "pierce", name: "신성 일격", armorPierce: 4 },
  },
  "별빛 수문장": {
    name: "별빛 수문장",
    tags: ["golem"],
    hp: 298, // 350→298(−15%, 2026-06-22): 수호 석상과 동일 — 잊힌 성소 반물리 절벽 완화(테마 유지·HP만).
    atk: 18,
    def: 15,
    spd: 3,
    exp: 30,
    skill: { kind: "heavy_blow", name: "내리찍기", everyPhases: 3, multiplier: 1.5 },
  },

  // ── 프론티어 밴드 E — 리자드 늪지 (깊이 29~35). water·earth, DoT 늪. ──────
  "리자드맨 전사": {
    name: "리자드맨 전사",
    tags: ["humanoid"],
    hp: 250,
    atk: 23,
    def: 11,
    spd: 6,
    exp: 26,
    evasionPct: 10,
    skill: { kind: "pierce", name: "창 찌르기", armorPierce: 3 },
  },
  "늪 독수": { name: "늪 독수", tags: ["beast"], hp: 230, atk: 20, def: 8, spd: 6, exp: 25, evasionPct: 13 },
  "수렁 거머리": { name: "수렁 거머리", tags: ["slime"], hp: 300, atk: 16, def: 13, spd: 3, exp: 27 },
  "독안개 정령": { name: "독안개 정령", tags: ["spirit"], hp: 185, atk: 23, def: 6, spd: 7, exp: 24, evasionPct: 17, atkType: "magic" },
  "늪지 도마뱀왕": {
    name: "늪지 도마뱀왕",
    tags: ["dragon"],
    hp: 290,
    atk: 25,
    def: 13,
    spd: 5,
    exp: 30,
    evasionPct: 11,
    skill: { kind: "heavy_blow", name: "꼬리치기", everyPhases: 3, multiplier: 1.5 },
    v2Skills: { learned: ["mob_crushing_blow"], equipped: ["mob_crushing_blow"] },
    v2MaxMp: 90,
  },

  // ── 프론티어 밴드 F — 짐승의 소굴 (깊이 36+). 자연 혼합+void 정예, 고공격·관통 강. ──────
  "거대 곰": {
    name: "거대 곰",
    tags: ["beast"],
    hp: 350,
    atk: 19,
    def: 15,
    spd: 3,
    exp: 29,
    evasionPct: 6,
    skill: { kind: "heavy_blow", name: "후려치기", everyPhases: 3, multiplier: 1.5 },
    // 거대 곰 — 포효(자버프)로 공격력을 올린 뒤 분쇄 일격. 우선순위 = equipped 배열 순서.
    v2Skills: {
      learned: ["mob_savage_roar", "mob_crushing_blow"],
      equipped: ["mob_savage_roar", "mob_crushing_blow"],
    },
    v2MaxMp: 90,
  },
  "우두머리 늑대": {
    name: "우두머리 늑대",
    tags: ["beast"],
    hp: 270,
    atk: 24,
    def: 9,
    spd: 8,
    exp: 28,
    evasionPct: 20,
    critPct: 30,
    skill: { kind: "pierce", name: "송곳니", armorPierce: 4 },
  },
  "화염 표범": {
    name: "화염 표범",
    tags: ["beast"],
    hp: 210,
    atk: 26,
    def: 6,
    spd: 7,
    exp: 27,
    evasionPct: 25,
    skill: { kind: "pierce", name: "할퀴기", armorPierce: 4 },
  },
  "뇌격 들소": {
    name: "뇌격 들소",
    tags: ["beast"],
    hp: 330,
    atk: 21,
    def: 14,
    spd: 4,
    exp: 30,
    evasionPct: 6,
    skill: { kind: "heavy_blow", name: "들이받기", everyPhases: 3, multiplier: 1.6 },
  },
  "공허 야수": {
    name: "공허 야수",
    tags: ["beast"],
    hp: 290,
    atk: 25,
    def: 12,
    spd: 6,
    exp: 32,
    evasionPct: 20,
    skill: { kind: "pierce", name: "그림자 발톱", armorPierce: 5 },
  },

  // ── 프론티어 밴드 G — 검은 왕도 (깊이 43~48). 엔드 확장 램프(권장 전투력 1500+) 전용. ──────
  "왕도 척후병": {
    name: "왕도 척후병",
    tags: ["humanoid"],
    hp: 235,
    atk: 27,
    def: 8,
    spd: 9,
    exp: 27,
    evasionPct: 22,
    critPct: 30,
    skill: { kind: "pierce", name: "섬광 찌르기", armorPierce: 5 },
  },
  "철갑 수문장": {
    name: "철갑 수문장",
    tags: ["golem"],
    hp: 380,
    atk: 22,
    def: 17,
    spd: 3,
    exp: 30,
    evasionPct: 4,
    skill: { kind: "heavy_blow", name: "왕도 분쇄", everyPhases: 3, multiplier: 1.6 },
    v2Skills: { learned: ["mob_crushing_blow"], equipped: ["mob_crushing_blow"] },
    v2MaxMp: 90,
  },
  "공허 기사": {
    name: "공허 기사",
    tags: ["humanoid", "undead"],
    hp: 310,
    atk: 26,
    def: 13,
    spd: 6,
    exp: 31,
    evasionPct: 14,
    skill: { kind: "pierce", name: "공허 절단", armorPierce: 5 },
    v2Skills: {
      learned: ["mob_savage_roar", "mob_crushing_blow"],
      equipped: ["mob_savage_roar", "mob_crushing_blow"],
    },
    v2MaxMp: 90,
  },
  "별사냥 까마귀": {
    name: "별사냥 까마귀",
    tags: ["beast"],
    hp: 220,
    atk: 25,
    def: 7,
    spd: 10,
    exp: 28,
    evasionPct: 26,
    atkType: "magic",
  },
  "심연 집행자": {
    name: "심연 집행자",
    tags: ["undead"],
    hp: 285,
    atk: 29,
    def: 10,
    spd: 7,
    exp: 32,
    evasionPct: 18,
    atkType: "magic",
    critPct: 20,
  },

  // ── 프론티어 밴드 H — 붉은 벌판 (깊이 49~54). 전쟁터·화염·쇳가루, 권장 전투력 1400~1650. ──
  "잿빛 창병": {
    name: "잿빛 창병",
    tags: ["humanoid"],
    hp: 260,
    atk: 28,
    def: 11,
    spd: 7,
    exp: 30,
    evasionPct: 14,
    skill: { kind: "pierce", name: "재 묻은 창끝", armorPierce: 5 },
  },
  "화약 투척병": {
    name: "화약 투척병",
    tags: ["humanoid"],
    hp: 230,
    atk: 30,
    def: 7,
    spd: 7,
    exp: 30,
    evasionPct: 16,
    atkType: "magic",
    skill: { kind: "pierce", name: "파편 폭발", armorPierce: 4 },
  },
  "붉은 갈기 늑대": {
    name: "붉은 갈기 늑대",
    tags: ["beast"],
    hp: 245,
    atk: 31,
    def: 8,
    spd: 10,
    exp: 31,
    evasionPct: 25,
    critPct: 30,
    skill: { kind: "pierce", name: "불붙은 송곳니", armorPierce: 5 },
  },
  "전장 철갑소": {
    name: "전장 철갑소",
    tags: ["beast"],
    hp: 390,
    atk: 24,
    def: 18,
    spd: 3,
    exp: 33,
    evasionPct: 5,
    skill: { kind: "heavy_blow", name: "철갑 돌진", everyPhases: 3, multiplier: 1.6 },
    v2Skills: { learned: ["mob_crushing_blow"], equipped: ["mob_crushing_blow"] },
    v2MaxMp: 90,
  },
  "폭풍 깃발수": {
    name: "폭풍 깃발수",
    tags: ["humanoid"],
    hp: 275,
    atk: 28,
    def: 10,
    spd: 9,
    exp: 32,
    evasionPct: 20,
    critPct: 20,
    v2Skills: {
      learned: ["mob_savage_roar", "mob_crushing_blow"],
      equipped: ["mob_savage_roar", "mob_crushing_blow"],
    },
    v2MaxMp: 90,
  },

  // ── 프론티어 밴드 I — 백골 고원 (깊이 55~60). 지상 엔드 고지대, 권장 전투력 1700~2000. ──
  "해골 기수": {
    name: "해골 기수",
    tags: ["undead"],
    hp: 275,
    atk: 32,
    def: 9,
    spd: 11,
    exp: 32,
    evasionPct: 27,
    critPct: 25,
    skill: { kind: "pierce", name: "마른 창격", armorPierce: 6 },
  },
  "고원 학살자": {
    name: "고원 학살자",
    tags: ["humanoid"],
    hp: 320,
    atk: 34,
    def: 12,
    spd: 7,
    exp: 34,
    evasionPct: 16,
    critPct: 30,
    skill: { kind: "heavy_blow", name: "처형 내려치기", everyPhases: 3, multiplier: 1.65 },
  },
  "뼈갑옷 들소": {
    name: "뼈갑옷 들소",
    tags: ["beast"],
    hp: 440,
    atk: 27,
    def: 20,
    spd: 3,
    exp: 35,
    evasionPct: 4,
    skill: { kind: "heavy_blow", name: "뼈갑옷 돌파", everyPhases: 3, multiplier: 1.65 },
    v2Skills: { learned: ["mob_crushing_blow"], equipped: ["mob_crushing_blow"] },
    v2MaxMp: 90,
  },
  "까마귀 주술사": {
    name: "까마귀 주술사",
    tags: ["humanoid"],
    hp: 250,
    atk: 33,
    def: 8,
    spd: 8,
    exp: 33,
    evasionPct: 22,
    atkType: "magic",
    critPct: 20,
  },
  "백골 군주": {
    name: "백골 군주",
    tags: ["undead", "humanoid"],
    hp: 370,
    atk: 35,
    def: 15,
    spd: 6,
    exp: 36,
    evasionPct: 14,
    critPct: 25,
    skill: { kind: "heavy_blow", name: "군주의 일격", everyPhases: 3, multiplier: 1.7 },
    v2Skills: {
      learned: ["mob_savage_roar", "mob_crushing_blow"],
      equipped: ["mob_savage_roar", "mob_crushing_blow"],
    },
    v2MaxMp: 110,
  },

  // ── 프론티어 밴드 J — 폭풍 산맥 (깊이 61~66). 고산 폭풍, 권장 전투력 2050~2250. ──
  "뇌운 절벽늑대": {
    name: "뇌운 절벽늑대",
    tags: ["beast"],
    hp: 300,
    atk: 36,
    def: 10,
    magicDef: 8,
    spd: 12,
    exp: 35,
    accuracy: 48,
    evasionPct: 30,
    critPct: 30,
    skill: { kind: "pierce", name: "폭풍 송곳니", armorPierce: 6 },
  },
  "폭풍뿔 거수": {
    name: "폭풍뿔 거수",
    tags: ["beast"],
    hp: 470,
    atk: 29,
    def: 21,
    magicDef: 14,
    spd: 4,
    exp: 37,
    accuracy: 30,
    evasionPct: 5,
    statusDamageReductionPct: 20,
    skill: { kind: "heavy_blow", name: "천둥 돌진", everyPhases: 3, multiplier: 1.7 },
    v2Skills: { learned: ["mob_crushing_blow"], equipped: ["mob_crushing_blow"] },
    v2MaxMp: 110,
  },
  "칼바람 약탈대장": {
    name: "칼바람 약탈대장",
    tags: ["humanoid"],
    hp: 340,
    atk: 36,
    def: 13,
    magicDef: 10,
    spd: 8,
    exp: 36,
    accuracy: 42,
    evasionPct: 18,
    statusDamageReductionPct: 5,
    critPct: 35,
    skill: { kind: "pierce", name: "절벽 급습", armorPierce: 6 },
  },
  "바위등 수호수": {
    name: "바위등 수호수",
    tags: ["beast"],
    hp: 500,
    atk: 30,
    def: 22,
    magicDef: 18,
    spd: 3,
    exp: 38,
    accuracy: 28,
    evasionPct: 4,
    statusDamageReductionPct: 25,
    skill: { kind: "heavy_blow", name: "산맥 후려치기", everyPhases: 3, multiplier: 1.75 },
    v2Skills: {
      learned: ["mob_savage_roar", "mob_crushing_blow"],
      equipped: ["mob_savage_roar", "mob_crushing_blow"],
    },
    v2MaxMp: 120,
  },
  "낙뢰 예언자": {
    name: "낙뢰 예언자",
    tags: ["humanoid"],
    hp: 280,
    atk: 37,
    def: 9,
    magicDef: 18,
    spd: 9,
    exp: 36,
    accuracy: 52,
    evasionPct: 23,
    statusDamageReductionPct: 10,
    atkType: "magic",
    critPct: 20,
  },

  // ── 프론티어 밴드 K — 심해 폐허 (깊이 67~72). 물·공허·별빛, 권장 전투력 2300~2500. ──
  "해연 추적자": {
    name: "해연 추적자",
    tags: ["dragon"],
    hp: 390,
    atk: 38,
    def: 14,
    magicDef: 16,
    spd: 8,
    exp: 38,
    accuracy: 50,
    evasionPct: 18,
    statusDamageReductionPct: 10,
    atkType: "magic",
    skill: { kind: "pierce", name: "물결 절단", armorPierce: 6 },
  },
  "청독 집게왕": {
    name: "청독 집게왕",
    tags: ["beast"],
    hp: 430,
    atk: 37,
    def: 18,
    magicDef: 14,
    spd: 6,
    exp: 39,
    accuracy: 38,
    evasionPct: 12,
    statusDamageReductionPct: 25,
    critPct: 25,
    skill: { kind: "pierce", name: "폐허 독침", armorPierce: 7 },
  },
  "침몰한 원혼": {
    name: "침몰한 원혼",
    tags: ["undead"],
    hp: 310,
    atk: 40,
    def: 9,
    magicDef: 22,
    spd: 10,
    exp: 38,
    accuracy: 48,
    evasionPct: 28,
    statusDamageReductionPct: 30,
    atkType: "magic",
    critPct: 25,
  },
  "해구의 사도": {
    name: "해구의 사도",
    tags: ["humanoid", "undead"],
    hp: 300,
    atk: 41,
    def: 8,
    magicDef: 20,
    spd: 8,
    exp: 39,
    accuracy: 55,
    evasionPct: 22,
    statusDamageReductionPct: 15,
    atkType: "magic",
    v2Skills: {
      learned: ["mob_savage_roar", "mob_crushing_blow"],
      equipped: ["mob_savage_roar", "mob_crushing_blow"],
    },
    v2MaxMp: 120,
  },
  "암류 파수병": {
    name: "암류 파수병",
    tags: ["humanoid", "undead"],
    hp: 420,
    atk: 39,
    def: 17,
    magicDef: 16,
    spd: 7,
    exp: 40,
    accuracy: 42,
    evasionPct: 15,
    statusDamageReductionPct: 20,
    critPct: 25,
    skill: { kind: "heavy_blow", name: "심연 참격", everyPhases: 3, multiplier: 1.75 },
    v2Skills: { learned: ["mob_crushing_blow"], equipped: ["mob_crushing_blow"] },
    v2MaxMp: 120,
  },

  // ── 프론티어 밴드 L — 천공 균열 (깊이 73~78). 6T 전환, 권장 전투력 4650~5500. ──
  "중력핵 골렘": {
    name: "중력핵 골렘",
    tags: ["golem"],
    hp: 520,
    atk: 43,
    def: 24,
    magicDef: 18,
    spd: 3,
    exp: 44,
    accuracy: 40,
    evasionPct: 4,
    statusDamageReductionPct: 35,
    skill: { kind: "heavy_blow", name: "중력 붕괴", everyPhases: 3, multiplier: 1.85 },
  },
  "붕괴의 선봉장": {
    name: "붕괴의 선봉장",
    tags: ["humanoid"],
    hp: 380,
    atk: 48,
    def: 15,
    magicDef: 12,
    spd: 9,
    exp: 43,
    accuracy: 58,
    evasionPct: 16,
    statusDamageReductionPct: 15,
    critPct: 35,
    skill: { kind: "pierce", name: "균열 돌파", armorPierce: 9 },
  },
  "무풍 추적귀": {
    name: "무풍 추적귀",
    tags: ["undead"],
    hp: 330,
    atk: 45,
    def: 10,
    magicDef: 15,
    spd: 13,
    exp: 43,
    accuracy: 62,
    evasionPct: 35,
    statusDamageReductionPct: 20,
    critPct: 35,
    skill: { kind: "pierce", name: "무풍 암습", armorPierce: 8 },
  },
  "만독 비룡": {
    name: "만독 비룡",
    tags: ["dragon"],
    hp: 450,
    atk: 44,
    def: 16,
    magicDef: 20,
    spd: 9,
    exp: 44,
    accuracy: 54,
    evasionPct: 18,
    statusDamageReductionPct: 40,
    critPct: 20,
    skill: { kind: "enrage", name: "맹독 폭주", hpFraction: 0.4, atkBonus: 10 },
  },
  "뇌정 성역지기": {
    name: "뇌정 성역지기",
    tags: ["spirit"],
    hp: 360,
    atk: 49,
    def: 12,
    magicDef: 28,
    spd: 10,
    exp: 45,
    accuracy: 64,
    evasionPct: 20,
    statusDamageReductionPct: 30,
    atkType: "magic",
    critPct: 25,
    v2Skills: {
      learned: ["mob_savage_roar", "mob_crushing_blow"],
      equipped: ["mob_savage_roar", "mob_crushing_blow"],
    },
    v2MaxMp: 140,
  },

  // ── 프론티어 밴드 M — 별의 무덤 (깊이 79~84). 6T 완성·유니크 추격. ──
  "성해의 파수꾼": {
    name: "성해의 파수꾼",
    tags: ["golem", "spirit"],
    hp: 620,
    atk: 52,
    def: 36,
    magicDef: 13,
    spd: 3,
    exp: 44,
    accuracy: 48,
    evasionPct: 5,
    skill: {
      kind: "heavy_blow",
      name: "성해 압괴",
      everyPhases: 3,
      multiplier: 2,
    },
  },
  "혜성꼬리 추적자": {
    name: "혜성꼬리 추적자",
    tags: ["beast"],
    hp: 340,
    atk: 55,
    def: 11,
    magicDef: 15,
    spd: 15,
    exp: 43,
    accuracy: 70,
    evasionPct: 42,
    critPct: 38,
    skill: { kind: "pierce", name: "혜성 관통", armorPierce: 11 },
  },
  "적색거성의 사제": {
    name: "적색거성의 사제",
    tags: ["humanoid", "spirit"],
    hp: 320,
    atk: 63,
    def: 8,
    magicDef: 23,
    spd: 10,
    exp: 43,
    accuracy: 68,
    evasionPct: 16,
    atkType: "magic",
    critPct: 28,
    v2Skills: {
      learned: ["mob_arcane_burst"],
      equipped: ["mob_arcane_burst"],
    },
    v2MaxMp: 160,
  },
  "공허를 먹는 짐승": {
    name: "공허를 먹는 짐승",
    tags: ["beast"],
    hp: 560,
    atk: 54,
    def: 20,
    magicDef: 20,
    spd: 8,
    exp: 44,
    accuracy: 60,
    evasionPct: 17,
    critPct: 22,
    skill: {
      kind: "enrage",
      name: "공허 포식",
      hpFraction: 0.4,
      atkBonus: 12,
    },
  },
  "죽은 별의 관측자": {
    name: "죽은 별의 관측자",
    tags: ["undead", "spirit"],
    hp: 390,
    atk: 59,
    def: 9,
    magicDef: 38,
    spd: 9,
    exp: 45,
    accuracy: 70,
    evasionPct: 20,
    atkType: "magic",
    critPct: 24,
    v2Skills: {
      learned: ["mob_arcane_burst"],
      equipped: ["mob_arcane_burst"],
    },
    v2MaxMp: 160,
  },

  // ── 훈련장 허수아비 — dungeon 을 안 거치고 spar 가 직접 조회. HP 고정 샌드백. ──
  "훈련용 허수아비": {
    name: "훈련용 허수아비",
    tags: ["humanoid"],
    image: "/images/monster/scarecrow.webp",
    hp: 500000,
    atk: 4,
    def: 2,
    spd: 1,
    exp: 0,
  },
};

// 훈련장 더미 id — 옛 monsters/index 의 SPAR_DUMMY_ID 와 동일 키, v2 소유로 이관.
export const V2_SPAR_DUMMY_ID = "훈련용 허수아비";
