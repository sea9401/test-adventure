...
export type V2CommonSkillId =
  ...
  | "v2c_warden_aegis" // 수호의 방벽 (보호막 — 최대HP 15%)
...
export const V2_COMMON_SKILLS: Record<V2CommonSkillId, V2SkillDefinition> = {
  ...
  v2c_warden_aegis: {
    // 수호의 방벽 — 최대 HP 15% 보호막(기존 shield effect 재사용·마나 보호막 패턴). 방어 탱의
  ...
  v2c_warden_thorns: {
    ...
    //   엔진 thornsFlatFromDef 훅(derive 가 def×thornsDefPct% 환산·enemyPhase 가 가산). PvE 전용
    //   (절정 반격과 동일 스코프). 방어=딜로 전환되는 탱딜 시너지(방벽 방어%와 결합).
