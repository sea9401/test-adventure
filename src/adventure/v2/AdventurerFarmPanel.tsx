...
export function AdventurerFarmPanel({ onBack }: { onBack: () => void }) {
  ...
  const toast = useMemo<FarmToast | null>(() => {
    ...
    return {
      id: notice.id,
      tone: "ok",
      text: `${notice.result.title} 납품 완료. ${formatSeedRewards(
        notice.result.rewardSeeds,
      )}와 농장 명성 ${notice.result.rewardReputation}을 받았습니다.`,
    };
  }, [notice]);

  useEffect(() => {
    if (!notice) return;
    const id = window.setTimeout(clearNotice, FARM_TOAST_MS);
