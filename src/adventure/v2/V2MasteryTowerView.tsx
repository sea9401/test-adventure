    ...
    <main className="mx-auto max-w-[720px] space-y-4 p-6 text-zinc-900 dark:text-zinc-100">
            ...
            <div className="rounded-md border border-zinc-200 bg-zinc-50 p-3 text-sm dark:border-zinc-700 dark:bg-zinc-900/60">
              {status.nextFloor == null ? (
                ...
                <div>
                  ...
                  {status.nextEncounter?.boss ? (
                    <div className="mt-2 space-y-1.5 rounded-md border border-amber-200 bg-amber-50 px-2 py-1.5 text-xs text-amber-800 dark:border-amber-900/70 dark:bg-amber-950/30 dark:text-amber-200">
                      <span className="font-semibold">보스 층</span>{" "}
                      {status.nextEncounter.boss.name} ·{" "}
                      {status.nextEncounter.boss.description}
                      {status.nextEncounter.gimmick ? (
                        <BossGimmick gimmick={status.nextEncounter.gimmick} />
                      ) : null}
                    </div>
                  ) : null}
                </div>
      ...
      {status && (
        <Card padding="md" className="space-y-2">
            ...
            {status.rewards.samples.map((row) => (
              ...
              >
                <div className="font-semibold">
                  {row.floor}층{row.boss ? " · 보스" : ""}
                </div>
                <div className="text-zinc-500 dark:text-zinc-400">
                  {row.reward.toLocaleString("ko-KR")}
                </div>
                <div className="mt-1 text-[11px] text-zinc-500 dark:text-zinc-400">
                  권장 {row.requiredPower.toLocaleString("ko-KR")}
                </div>
                {row.boss ? (
                  <div className="mt-1 truncate text-[11px] font-medium text-amber-700 dark:text-amber-300">
                    {row.boss.name}
                  </div>
                ) : null}
                {row.gimmick ? (
                  <div className="mt-1 text-[10px] text-amber-700 dark:text-amber-300">
                    기믹 {row.gimmick.penaltyPercent > 0 ? `+${row.gimmick.penaltyPercent}%` : "파훼"}
                  </div>
                ) : null}
              </div>
            ))}
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            보스 층:{" "}
            {status.rewards.bosses
              .map((boss) => `${boss.floor}층 ${boss.name}`)
              .join(" · ")}
          </p>
        </Card>
      )}
...
function BossGimmick({ gimmick }: { gimmick: TowerGimmick }) {
  return (
    <div className="mt-1 border-t border-amber-200 pt-1 dark:border-amber-900/70">
      <div className="font-semibold">
        {gimmick.name}
      ...
      <div className="mt-1 flex flex-wrap gap-1">
        {gimmick.checks.map((check) => (
          ...
          >
            ...
            {check.target.toLocaleString("ko-KR")}
          </span>
        ))}
      </div>
