// 투기장 코인 상점 카탈로그 — 투기장 코인(pvp-wallet.v1)으로 구매. 주력은 코스메틱(칭호) —
// 주간 시즌 순위 보상 화폐의 소비처. 소비템(스태미나 회복약)은 보관형 예외로 취급한다.
...
export type ArenaShopConsumable = {
  itemId: string;
  name: string;
  description: string;
  price: number;
...
export const ARENA_SHOP_CONSUMABLES: readonly ArenaShopConsumable[] = [
  {
...
export function arenaShopConsumablePriceFor(itemId: string): number | undefined {
  return ARENA_SHOP_CONSUMABLES.find((item) => item.itemId === itemId)?.price;
}
