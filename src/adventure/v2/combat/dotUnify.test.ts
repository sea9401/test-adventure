...
describe("PR-2 DoT 피해 공식", () => {
  ...
  it("중독 = %최대HP, 저HP 적은 상한 미적용(HP 비례 그대로)", () => {
    const dot = makePoisonDot({ pctMaxHpPerStack: 0.005, sourceAtk: 100 });
    // maxHp 1000 × 0.005 = 5, 상한 100×0.9=90 → min = 5 (HP 비례).
    expect(v2DotPerStackDamage(dot, 1000)).toBeCloseTo(5);
  });

  it("중독 보스 A상한 — 고HP 적은 ATK×k 로 상한(과녹임 차단)", () => {
    const dot = makePoisonDot({ pctMaxHpPerStack: 0.005, sourceAtk: 100 });
    // maxHp 50000 × 0.005 = 250 이지만 상한 100×0.9=90 → 90 으로 캡.
...
describe("PR-2 DoT tag 분리·누적", () => {
  it("출혈·중독 공존 (다른 tag)", () => {
    const out = applyV2DotsToTarget(
      [],
      [
        makeBleedDot({ flatPerStack: 10, sourceAtk: 100 }),
        makePoisonDot({ pctMaxHpPerStack: 0.005, sourceAtk: 100 }),
      ],
    );
