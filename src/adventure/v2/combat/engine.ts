...
import { hasUnexploredEffect, recordUnexploredEnemySkillHits, unyieldingDamagePve } from "./unexploredSetPveAdapter";
...
export { beginUnexploredPlayerAttack,finishUnexploredEnemyAction,finishUnexploredFrostEnemyAction,finishUnexploredPlayerAttack,hasUnexploredEffect,recordUnexploredEnemyHit,recordUnexploredEnemySkillHits,sumMagicBarrierDamage,unexploredDefensePve,unyieldingDamagePve } from "./unexploredSetPveAdapter";
...
import { applyV2BuffsToMap, applyV2DotsToTarget, DAMAGE_FLOOR_FRACTION, damageBetween, distributeV2DotTicks, resolveV2SkillCast, statusDamageAfterReduction, tickV2BuffMap, tickV2Dots, v2DotLogCause } from "./combatShared";
import { resolveBattleAtb } from "./engine.atb";
import { appendEnemySkillMitigationLogs, evadeIncomingEnemySkill, resolveIncomingEnemySkillWithBarrier } from "./engine.enemySkills";
...
import { applyBerserkerHostileDamage, applyTrackedSetShieldAbsorptionPve, finishPlayerTurn, initialBattleState, rollPlayerAttackCountWithBleed } from "./engine.pveOperations";
...
import { type BattleLogEntry, type BattleState, type PlayerAction, type PlayerCombat } from "./engineState";
...
import { consumePurificationWard, TRIPLE_WARD_LABELS } from "./tripleWard";

export { BOSS_MAX_HP_DAMAGE_MULT, BOSS_PCT_HP_DAMAGE_MULT, COMBO_FINISHER_PERIOD } from "./engineState";
...
function resolveBattleLegacy(
  ...
  let generatedBasicsThisPlayerPhase = false;
  ...
  while (state.phase !== "ended") {
    ...
    if (state.phase !== "player") generatedBasicsThisPlayerPhase = false;
    ...
    if (state.phase === "player") {
      ...
      if (!v2CastedThisPlayerPhase) {
        ...
        let playerDotDamage = statusDamageAfterReduction(
          playerDotTick.totalDmg,
          player.statusDamageReductionPct,
        );
        if (hasUnexploredEffect(player, "unyielding_dead")) {
          let hpBefore = state.playerHp;
          playerDotDamage = playerDotTick.ticks.reduce((sum, tick) => {
            const damage = statusDamageAfterReduction(unyieldingDamagePve({ ...state, playerHp: hpBefore }, player, tick.damage), player.statusDamageReductionPct);
            hpBefore = Math.max(0, hpBefore - damage);
            return sum + damage;
          }, 0);
        }
      ...
      if (!v2CastedThisEnemyPhase) {
        ...
        const countered =
          canMartialCounterHit(player, enemySkillDamageToHp, enemySkillShieldAbsorbed, enemySkillMagicBarrier.absorbedDamage) && result.castSkillName
            ? applyPassiveCounterOnHitIfAny(
                {
                  ...state,
                  stacks: { ...state.stacks, fortressImpact: legacyFortressReaction.impact },
        ...
        state = {
          ...
          stacks: {
            ...
            fortressImpact: countered?.stacks.fortressImpact ?? legacyFortressReaction.impact,
        ...
        state = recordUnexploredEnemySkillHits(state, player, skillHits, actualEnemySkillHpDamage);
        if (
    ...
    state = advanceTurn(
      ...
      generatedBasicsThisPlayerPhase ? "extra_basic" : undefined,
    );
    if (action.kind === "use_potion" && state.phase === "player") {
      generatedBasicsThisPlayerPhase = true;
    }
...
export { applyPassiveCounterOnHitIfAny } from "./engine.passiveCounter";
