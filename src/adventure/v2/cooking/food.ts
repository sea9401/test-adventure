...
export type CookingFoodId =
  `food2:${string}:${CookingQuality}:o${0 | 1}:s${0 | 1 | 2 | 3 | 4 | 5}`;
...
const QUALITY_PERFORMANCE: Record<CookingQuality, number> = {
  normal: 0,
  careful: 10,
  masterpiece: 20,
};
const QUALITY_DELIVERY: Record<CookingQuality, 100 | 125 | 160> = {
  normal: 100,
  careful: 125,
  masterpiece: 160,
};
const COMBAT_CAPS: Record<CookingCombatFlatKey, number> = {
  atk: 300,
  magicAtk: 300,
  def: 300,
  magicDef: 300,
...
function boundedDecimal(raw: unknown, cap: number): number {
  return Math.min(cap, Math.max(0, Math.round((Number(raw) || 0) * 10) / 10));
}

function scaledRecord<K extends string>(
  ...
  const entries = Object.entries(source).flatMap(([key, raw]) => {
    const value = decimal
      ? boundedDecimal(Number(raw) * multiplier, capFor(key as K))
      : boundedInt(Number(raw) * multiplier, capFor(key as K));
...
export function cookingPerformancePct(args: {
  quality: CookingQuality;
  originator: boolean;
  specialtyBonusPct: number;
}): number {
  return Math.min(
    135,
    100 +
      QUALITY_PERFORMANCE[args.quality] +
      (args.originator ? 10 : 0) +
      Math.min(5, Math.max(0, Math.floor(args.specialtyBonusPct))),
  );
}

export function scaleCookingEffect(
  effect: CookingEffect,
  args: {
    quality: CookingQuality;
    originator: boolean;
  ...
  const combatFlat = scaledRecord<CookingCombatFlatKey>(
    effect.combatFlat,
    multiplier,
    (key) => COMBAT_CAPS[key],
    false,
  ...
  return {
    ...
    ...(combatFlat ? { combatFlat } : {}),
    ...(huntExpPct > 0 ? { huntExpPct } : {}),
    ...(huntGoldPct > 0 ? { huntGoldPct } : {}),
    ...(cookingXpPct > 0 ? { cookingXpPct } : {}),
  };
}
...
export function parseCookingFoodId(raw: unknown): CookingFoodVariant | null {
  ...
  if (prefix !== "food2" || extra !== undefined || !COOKING_PUBLIC_RECIPE_BY_ID.has(recipeId)) return null;
  const quality: CookingQuality | null =
    qualityRaw === "normal" || qualityRaw === "careful" || qualityRaw === "masterpiece"
      ? qualityRaw
      : null;
  if (!quality || (originRaw !== "o0" && originRaw !== "o1") || !/^s[0-5]$/.test(specialtyRaw)) return null;
  return {
    id: raw as CookingFoodId,
    recipeId,
    quality,
    originator: originRaw === "o1",
    specialtyBonusPct: Number(specialtyRaw.slice(1)) as 0 | 1 | 2 | 3 | 4 | 5,
  };
}
...
export function cookingFoodDefinition(raw: unknown): CookingFoodDefinition | null {
  ...
  return {
    ...
    deliveryScorePct: QUALITY_DELIVERY[variant.quality],
...
export function activeCookingBuff(raw: unknown, now = Date.now()): ActiveCookingBuff | null {
  ...
  const recipe = typeof value.recipeId === "string"
    ? COOKING_PUBLIC_RECIPE_BY_ID.get(value.recipeId)
    : null;
  if (!recipe || !Number.isFinite(value.expiresAt) || Number(value.expiresAt) <= now) return null;
  const quality: CookingQuality = value.quality === "masterpiece" || value.quality === "careful"
    ? value.quality
    : "normal";
  ...
  return {
    ...
    quality,
    effect: scaleCookingEffect(source, {
      quality: "normal",
      originator: false,
      specialtyBonusPct: 0,
    }),
...
export function cookingEffectText(effect: CookingEffect): string {
  ...
  const labels: Record<CookingCombatFlatKey, string> = {
    atk: "공격력", magicAtk: "마법공격력", def: "방어력", magicDef: "마법방어력",
    maxHp: "최대 HP", maxMp: "최대 MP", accuracy: "적중",
  };
  for (const [key, value] of Object.entries(effect.combatFlat ?? {})) {
    if (value) parts.push(`${labels[key as CookingCombatFlatKey]} +${value}`);
  }
