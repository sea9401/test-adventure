...
vi.mock("@/lib/server/accountLinkIntent", () => ({
  ...
  isAccountLinkProvider: (value: unknown) => value === "kakao",
}));
...
describe("POST /api/auth/link", () => {
  ...
  it("로그인하지 않은 요청을 거절한다", async () => {
    mocks.userId = null;
    const response = await POST(request(JSON.stringify({ provider: "kakao" })));
  ...
  it("허용 목록 밖 provider와 잘못된 JSON을 거절한다", async () => {
    await expect(
      POST(request(JSON.stringify({ provider: "google" }))),
  ...
  it("사용자 ID 대신 서버가 만든 무작위 token만 HttpOnly 쿠키에 둔다", async () => {
    ...
    expect(mocks.createIntent).toHaveBeenCalledWith("user-1", "kakao");
