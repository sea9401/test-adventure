// 투기장 코인 상점 — 투기장 코인(pvp-wallet.v1)으로 칭호 구매. (낚시 코인 상점 미러.)
//
// GET  /api/v2/arena/shop — { coins, ownedTitleIds } (상점 UI 초기 상태).
// POST /api/v2/arena/shop — body { titleId } 구매.
//   1) 카탈로그 가격 조회 (미등재 → 400)
//   2) 트랜잭션: pvp-wallet.v1 잠금 → 코인 검증 → 칭호 부여(idempotent) → 코인 차감
//      - 코인 부족 → 402 (지급/부여 없음)
//      - 이미 보유 → 409 (차감 없음)
// 락 순서: pvp-wallet.v1 → adventure-log.v2(grantTitleIfMissingInTx 내부). 시즌 보상 지급
...
import { ARENA_SHOP_TITLES, arenaShopPriceFor } from "@/adventure/v2/arenaShop";
