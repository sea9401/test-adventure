    ...
    select: () => {
      ...
      const query = {
        ...
        for: async () => {
          ...
          if (h.publishOnLock) h.session.visibility = "public";
          return rows();
        },
...
describe("협동 무료 지원 공격", () => {
  beforeEach(() => {
    ...
    h.publishOnLock = false;
    vi.clearAllMocks();
  });
  it.each([
    ["public", false], ["public", true],
    ["guild_only", false], ["guild_only", true],
    ["summoner_only", false], ["summoner_only", true],
    [null, false],
  ])("공개 범위 %s, 지원 %s: 공개 보스만 처치 피드를 발행한다", async (visibility, support) => {
    h.session.visibility = visibility;
    h.session.summonerId = "helper";
    h.session.summonerGuildId = 1;
    h.char.stamina = { current: 100, lastUpdatedAt: Date.now() };
    const response = await POST(request(support));
    expect(response.status).toBe(200);
    expect(await response.json()).toMatchObject({
      result: { defeated: true, killingBlowReward: support ? null : { coin: 5 } },
    });
    if (visibility === "public" || visibility === null) {
      expect(h.feed).toHaveBeenCalledExactlyOnceWith("helper", "coop_kill", {
        kind: "mountain_chief", sessionId: "boss",
      });
    } else {
      expect(h.feed).not.toHaveBeenCalled();
    }
    expect(h.notifications).toHaveBeenCalledWith(
      ["helper"], "coop_defeated", expect.objectContaining({ sessionId: "boss" }),
    );
  });
  it("처치 전 잠금에서 확인한 전체 공개 전환을 피드에 반영한다", async () => {
    h.session.visibility = "summoner_only";
    h.session.summonerId = "helper";
    h.publishOnLock = true;
    expect((await POST(request(true))).status).toBe(200);
    expect(h.feed).toHaveBeenCalledTimes(1);
  });
  it("공개 보스도 살아 있으면 처치 피드와 개인 알림을 보내지 않는다", async () => {
    h.enemyHp = 50;
    expect((await POST(request(true))).status).toBe(200);
    expect(h.feed).not.toHaveBeenCalled();
