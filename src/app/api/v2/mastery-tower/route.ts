...
export async function GET() {
  ...
  return Response.json({
    ...
    nextRequiredPower:
      nextFloor == null
        ? null
        : masteryTowerRequiredPower(nextFloor, combatProfile),
    nextEncounter:
      nextFloor == null ? null : masteryTowerFloorInfo(nextFloor, combatProfile),
    rewards: {
      maxFloor: MASTERY_TOWER_MAX_FLOOR,
      milestones: MASTERY_TOWER_MILESTONES,
      bosses: MASTERY_TOWER_BOSSES,
      samples: [5, 10, 20, 30, 40, 50].map((floor) => ({
        floor,
        reward: masteryTowerFloorReward(floor),
        requiredPower: masteryTowerRequiredPower(floor, combatProfile),
        boss: masteryTowerFloorInfo(floor, combatProfile).boss,
        baseRequiredPower: masteryTowerFloorInfo(floor, combatProfile).baseRequiredPower,
        gimmick: masteryTowerFloorInfo(floor, combatProfile).gimmick,
      })),
    },
    jobs,
  });
