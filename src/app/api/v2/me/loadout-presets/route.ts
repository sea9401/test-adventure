...
// 현재 프리셋 상태 — 슬롯/사용분/잔액 포함(파생, skills.v2 기준).
function presetStateOf(skills: V2SkillsState) {
  ...
  const spent = presetSlotSpend(bought);
  return {
...
export async function POST(req: Request) {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    if (action === "buySlot") {
      ...
      const lifetime = countCollectedPassives(skills.learned);
      const available = Math.max(0, lifetime - presetSlotSpend(bought));
      if (available < cost) {
        return {
          status: 400,
          body: {
      ...
      return { status: 200, body: { ok: true as const, ...presetStateOf(next) } };
    }
