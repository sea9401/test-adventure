...
// POST /api/v2/me/spec — 전문화(스펙) 선택 + 차수별 패시브 픽.
//   body { specId }    → 전문화 선택(2차 도달부터, job 의 전문화 중 하나). 이미 다른 전문화면 잠김
//                        (신전 초기화로만 변경). 같은 전문화 재선택은 멱등.
//   body { passiveId } → 그 전문화 키트에서 패시브 1개 해금. 픽 한도 = tier-1(2차 1·3차 2·4차 3).
...
export async function POST(req: Request) {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    if (specId) {
      ...
      // 같은 전문화 재선택 = 멱등. 다른 전문화로 변경은 잠김(신전 초기화로만).
      ...
      if (curSpecId !== specId) {
        ...
        // 전문화 첫 선택 시 그 전문화 무기 1회 지급(게이트 활성화). 이미 보유면 skip.
