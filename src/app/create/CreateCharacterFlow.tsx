...
//  ② 레거시 모드만 시작 직업 선택 — /api/v2/me/class-element.
// 코어루프 운영에서는 모두 모험가(none)로 시작하므로 이름·외형을 정하면 바로 완료한다.
...
export function CreateCharacterFlow() {
  ...
  const finish = async () => {
    ...
    try {
      ...
      if (!j?.ok) {
        setErr(`직업 적용 실패: ${j?.error ?? `http ${res.status}`}`);
        setBusy(false);
        return;
      }
      router.replace("/");
        ...
        {phase === "profile" ? (
          <>
            ...
            <CreateCharacterForm
              onSubmit={submit}
              onSuccess={() => {
                if (V2_CORE_LOOP_V2) router.replace("/");
                else setPhase("class");
              }}
            />
          </>
          ...
          <>
            ...
            <p className="mb-4 mt-1 text-sm text-zinc-500 dark:text-zinc-400">
              시작 직업을 고르세요. 나중에 마을 「성장의 신전」에서 바꿀 수 있어요.
            </p>
