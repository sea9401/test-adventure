...
type CheckState =
  | { kind: "idle" }
  | { kind: "invalid"; reason: CharacterNameInvalidReason }
  | { kind: "checking" }
  | { kind: "available" }
  | { kind: "taken" }
  | { kind: "error" };
...
export function CreateCharacterForm({
  ...
  useEffect(() => {
    ...
    if (!validation.ok) {
      setCheck({ kind: "invalid", reason: validation.reason });
      return;
    }
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch(
          `/api/profile/check-name?name=${encodeURIComponent(normalizedName)}`,
        );
        if (cancelled) return;
        if (!res.ok) {
          setCheck({ kind: "error" });
        ...
        const data = (await res.json()) as {
          available: boolean;
          reason?: CharacterNameInvalidReason | "taken";
        };
        if (data.available) setCheck({ kind: "available" });
        else if (data.reason && data.reason !== "taken") {
          setCheck({ kind: "invalid", reason: data.reason });
        } else setCheck({ kind: "taken" });
      } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!canSubmit || avatar === null || !nameCheck.ok) return;
    setSubmitting(true);
    setRetrying(false);
    setSubmitError(null);
  ...
  if (state.kind === "idle") {
    return (
      <p className="mt-1.5 text-xs text-zinc-500 dark:text-zinc-400">
        {CHARACTER_NAME_RULE_TEXT} 다른 모험가와 겹칠 수 없습니다.
      </p>
    );
  }
  if (state.kind === "invalid") {
    return (
      <p className="mt-1.5 inline-flex items-center gap-1 text-xs text-rose-600 dark:text-rose-400">
        ...
        {characterNameInvalidMessage(state.reason)}
      </p>
    );
  }
