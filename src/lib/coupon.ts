...
export function parseCouponReward(value: unknown): CouponReward | null {
  ...
  // 쿠폰 발급 CLI 는 현재 수량형 보상만 만든다. 배열은 향후 관리자 UI가 생겨도 기존
  // admin_gift 계약을 그대로 쓸 수 있도록 빈 배열만 허용한다.
  if (
  ...
  if (gold + staminaPotions + museunCoins + adventureSupportDays <= 0) return null;
