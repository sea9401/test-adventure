...
describe("derivePlayerCombatV2Pure critResistPct (SPI PR-3b — 정신 치명저항)", () => {
  it("정신·장비·해방 치명저항을 50%p에서 자르지 않고 합산한다", () => {
    // spi 200 → 20%p.
    const mid = derivePlayerCombatV2Pure({
      level: 60,
    ...
    liberationEffects.combat.critResistPp = 25;
    const heavy = derivePlayerCombatV2Pure({
      level: 60,
      allocatedStats: { spi: 1000 },
      v2Equipped: { ring: "v2_crafted_oblivion_ring" },
      liberationEffects,
    }).player;
    expect(heavy.critResistPct).toBeCloseTo(101.5 + 45 + 25, 5);
  });
});
