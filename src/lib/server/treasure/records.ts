...
import { shapeTreasureLeaderboard } from "@/adventure/v2/treasureLeaderboard";
...
export async function getTreasureLeaderboard(
  ...
  return shapeTreasureLeaderboard(
    rows.map((r) => ({ userId: r.userId, name: r.name, value: r.value })),
    meUserId,
    topN,
  );
}
